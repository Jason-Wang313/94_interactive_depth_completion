        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Interactive Depth Completion: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study active perception for robotics. The motivating bet is: Complete depth by physically probing where perception uncertainty matters to action.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For active perception for robotics, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.253 & 0.748 \\
scalar\_uncertainty\_wam & nominal & 0.625 & 0.375 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.323 & 0.677 \\
proposed\_branch\_mechanism & nominal & 0.738 & 0.262 \\
...
\end{tabular}
        ...
        \section{Conclusion}
        Interactive Depth Completion argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
